...
const ReturnSection = ({ title, children, icon, gradient = "from-pink-50 via-white to-yellow-100", className = "" }) => {
    return (
        <section
        ...
        >
            ...
            <div className="flex-1">
                <h3 className="text-lg md:text-xl font-bold text-pink-800 mb-2 flex items-center gap-2">{title}</h3>
                <div className="text-gray-700 text-base md:text-lg">{children}</div>
            </div>
            {/* Hiệu ứng blob nền động */}
            <span className="absolute -top-8 -left-8 w-24 h-24 bg-pink-200 opacity-20 rounded-full blur-2xl animate-blob z-0" />
            <span className="absolute -bottom-8 -right-8 w-24 h-24 bg-yellow-200 opacity-10 rounded-full blur-2xl animate-blob2 z-0" />
        </section>
    );
};
