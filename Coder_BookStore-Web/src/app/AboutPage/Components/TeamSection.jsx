  ...
  return (
    <section className="py-12 px-4 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold text-purple-700 dark:text-yellow-300 mb-6 animate-fade-in font-heading">{content[lang].title}</h2>
          ...
          {content[lang].members.map((member, index) => (
            <div key={index} className="text-center animate-fade-in-slow">
              <div className="w-20 h-20 bg-gradient-to-br from-pink-200 via-yellow-200 to-purple-200 dark:from-pink-900 dark:via-yellow-900 dark:to-purple-900 rounded-full mx-auto mb-3 shadow-lg flex items-center justify-center">
                <span className="text-2xl font-bold text-gray-700 dark:text-white font-heading">{member.name[0]}</span>
              </div>
              <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-1 font-heading">{member.name}</h3>
              <p className="text-xs text-gray-600 dark:text-gray-300 font-body">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
