...
export default function FaqPage() {
  ...
  useEffect(() => {
    const fetchFaqs = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            "Không thể tải FAQ. Có thể server đang đi uống cà phê!"
          );
        }
  ...
  const categories = [
    "Tất cả",
    ...Array.from(new Set(faqs.map((f) => f.category || "Khác"))),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-pink-50 py-12 px-4">
      <FAQHeader />{" "}
      {/* Keep this if FAQHeader component exists and is desired */}
      ...
      <h1 className="text-4xl font-extrabold text-center mb-10 text-blue-700 drop-shadow">
        ...
        <span className="block text-base text-pink-500 mt-2 font-normal">
          Có thắc mắc thì hỏi, đừng ngại! Chúng tôi trả lời nghiêm túc, nhưng
          đôi khi cũng vui vẻ lắm 😎
        </span>
      </h1>
      ...
      <main className="max-w-3xl mx-auto px-4 md:px-0 relative z-10">
        ...
        {loading && (
          <div className="text-center text-blue-500 animate-pulse">
            Đang tải... Đợi xíu nha, FAQ đang khởi động não 🧠
          </div>
        )}
