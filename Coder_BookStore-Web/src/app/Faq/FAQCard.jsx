...
const FAQCard = ({ question, answer, isOpen, onClick, color }) => {
  ...
  return (
    <div className={`relative rounded-2xl shadow-lg p-6 md:p-8 transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl animate-fade-in cursor-pointer ${color || 'bg-white'}`}>
      <div className="flex items-center gap-3 mb-2" onClick={onClick}>
        ...
        <button
          className={`ml-2 w-8 h-8 flex items-center justify-center rounded-full border-2 border-blue-300 bg-white text-blue-700 font-bold text-xl transition-transform duration-200 ${isOpen ? 'rotate-45' : ''}`}
