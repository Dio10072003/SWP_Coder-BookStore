...
export default function OrderRating({ tracking }) {
  ...
  if (submitted)
    return (
      <div className="bg-green-50 rounded-xl p-4 text-green-700 font-semibold mt-6">
        Cảm ơn bạn đã đánh giá đơn hàng!
      </div>
    );
  return (
    <form
      className="bg-white rounded-xl shadow p-6 mt-6 flex flex-col gap-3"
      onSubmit={(e) => {
        e.preventDefault();
        setSubmitted(true);
      }}
...
const handleSubmit = async (e) => {
  e.preventDefault();
  if (rating === 0) return alert('Vui lòng chọn số sao để đánh giá.');
