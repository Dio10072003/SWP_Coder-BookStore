...
export default function PromotionDemo() {
  ...
  const handleUpdate = async () => {
    ...
    setActionLoading(true);
    try {
      await promotionService.updatePromotion(selectedId, { title: 'Đã cập nhật', discount: 99 });
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 bg-white rounded-xl shadow mt-8">
      ...
      <form onSubmit={handleCreate} className="flex flex-col gap-2 mb-6">
        ...
        <button type="submit" className="bg-pink-500 text-white px-4 py-2 rounded disabled:opacity-60" disabled={actionLoading}>{actionLoading ? 'Đang xử lý...' : 'Tạo mới'}</button>
      </form>
      <div className="mb-4">
      ...
      {selectedPromo && (
        <div className="flex gap-4 justify-center mt-4">
          <button onClick={handleUpdate} className="bg-yellow-400 px-5 py-2 rounded text-white font-bold disabled:opacity-60" disabled={actionLoading}>Cập nhật</button>
