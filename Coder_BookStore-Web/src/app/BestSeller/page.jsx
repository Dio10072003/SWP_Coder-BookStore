  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-3xl shadow-2xl p-8 max-w-2xl w-full mx-4 relative animate-fade-in-up">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-10 h-10 rounded-full bg-gray-100 hover:bg-red-100 flex items-center justify-center text-gray-500 hover:text-red-500 transition-all duration-300"
        >
          <FaTimes />
        </button>
    ...
    bookService
      ...
      .then(({ data }) => {
        ...
        const sorted = [...data].sort(
          (a, b) => b.rating - a.rating || b.id - a.id
        );
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center py-12">
        ...
        <p className="text-xl md:text-2xl text-gray-700 font-light mb-8 text-center max-w-4xl leading-relaxed">
          Khám phá những tựa sách công nghệ, lập trình được yêu thích nhất bởi
          cộng đồng{" "}
          <span className="text-purple-600 font-semibold">CoderTour</span>
        </p>
      </div>
...
function BookCard({ book, rank, onQuickView }) {
  ...
  return (
    ...
    >
      {/* Rank badge */}
      <div
        className={`absolute top-3 left-3 bg-gradient-to-r ${getRankColor(
          rank
        )} text-white px-3 py-1 rounded-full text-sm font-bold shadow-lg z-10`}
      >
        {getRankIcon(rank)}
      </div>
      ...
      <div className="p-6">
        ...
        <p className="text-gray-600 mb-3 text-sm">
          Tác giả:{" "}
          <span className="font-semibold text-blue-600">{book.author}</span>
        </p>
