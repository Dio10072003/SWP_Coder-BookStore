...
export default function Home() {
  ...
  return (
    <div className="min-h-screen px-6 py-12 max-w-7xl mx-auto">
      ...
      >
        <h1 className="text-5xl font-extrabold mb-4 tracking-wide text-pink-600 drop-shadow-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {books.map(({ id, title, author, price, img }) => (
            <motion.div
              ...
              className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow cursor-pointer group"
            >
              <Image
                src={img}
                alt={title}
              ...
              />
              <div className="p-5">
                <h3 className="text-xl font-bold mb-1 text-pink-600 dark:text-pink-400">{title}</h3>
      ...
      <motion.section
        ...
        className="mb-16"
      >
        <h2 className="text-3xl font-extrabold text-pink-700 dark:text-pink-400 mb-8 text-center drop-shadow-sm">
          Explore More
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {explore.map(({ href, icon, label, desc }) => (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center bg-gradient-to-tr from-yellow-300 via-pink-300 to-purple-300 dark:from-yellow-700 dark:via-pink-700 dark:to-purple-900 p-8 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 transition-transform cursor-pointer text-center text-purple-900 dark:text-yellow-300"
            >
